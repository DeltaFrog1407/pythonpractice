...
class Pipe():
    def __init__(self):
        ...
        pipes = ('물고기 게임 만들기/assets/pipe02.png', '물고기 게임 만들기/assets/pipe03.png',
                 '물고기 게임 만들기/assets/pipe04.png', '물고기 게임 만들기/assets/pipe05.png', '물고기 게임 만들기/assets/pipe06.png')
    ...
    def out_of_screen(self):
        if self.spipe_rect.x + self.spipe_rect.width <= 0:
            return True
        return False
    
    def check_crush(self, fish):
        if (self.lpipe_rect.x + self.lpipe_width > fish.rect.x) and (self.lpipe_rect.x < fish.rect.x + fish.width) \
             and (self.lpipe_rect.y < fish.rect.y + fish.height) and (self.lpipe_rect.y + self. lpipe_height > fish.rect.y):
                return True
        elif (self.spipe_rect.x + self.spipe_width > fish.rect.x) and (self.spipe_rect.x < fish.rect.x + fish.width) and \
            (self.spipe_rect.y < fish.rect.y + fish.height) and (self.spipe_rect.y + self. spipe_height > fish.rect.y):
                return True
        else:
            return False
...
class Game():
    def __init__(self):
        ...
        self.font = pygame.font.SysFont("FixedSys", 40, True, False)
        pygame.mixer.music.load(os.path.join(self.assets_path, 'bgm.mp3'))
        self.fish = Fish()
        self.pipes = []
    ...
    def run_logic(self, screen):
        for pipe in self.pipes:
            ...
            if pipe.out_of_screen():
                del self.pipes[0]
                self.pipe_pos = random.randrange(200, 400, 20)
